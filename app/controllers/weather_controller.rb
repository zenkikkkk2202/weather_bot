  ...
  def callback
    ...
    events.each { |event|
      ...
      if event.message['text'].include?("天気")
        ...
        response = `curl -X GET http://api.gnavi.co.jp/RestSearchAPI/v3/?keyid=161a20d6368441dd8e7d27c1aa717317'&'format=json'&'latitude=#{latitude}'&'longitude=#{longitude}`#ここでぐるなびAPIを叩く
      else
        # おうむ返し
        event.message['text']
