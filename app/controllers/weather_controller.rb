  ...
  def callback
    ...
    events.each { |event|
      ...
      if event.message['text'].include?("天気")
        ...
        response = store = `curl -X GET https://api.gnavi.co.jp/RestSearchAPI/v3/?keyid=161a20d6368441dd8e7d27c1aa717317&address=#{store}'&'format=json`
        # response = area = "https://api.gnavi.co.jp/RestSearchAPI/v3/?keyid=161a20d6368441dd8e7d27c1aa717317&address=#{area}format=json'&'address=#{place}`
      else
        # おうむ返し
        event.message['text']
